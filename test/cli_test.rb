...
Writing output to file: #{dir}/location.rbs
  ...
  def test_validate_options
    with_cli do |cli|
      err = assert_raises(ValidationError) { cli.run(%W(./schema/does_not_exist.json)) }
      assert_equal "./schema/does_not_exist.json: No such file or directory found!", err.message
    end

    with_cli do |cli|
      err = assert_raises(ValidationError) { cli.run(%W(./schema/does/not/exist)) }
      assert_equal "./schema/does/not/exist: No such file or directory found!", err.message
    end

    with_cli do |cli|
      err = assert_raises(ValidationError) { cli.run(%W(./schema/location.json -o /does/not/exist)) }
